Feature: Start Game with Shards Deduction
  ...
  Background:
    Given a server exists with name "Shard Server"
    And the server has max players 3
    And a user with email "creator@example.com" has 300 shards
    And a user with email "player1@example.com" has 200 shards
    And a user with email "player2@example.com" has 100 shards
  ...
  Scenario: Insufficient shards for some players
    ...
    When the server creator views the server page
    Then they should see "Not all players have enough shards to start the game"
